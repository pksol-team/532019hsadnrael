...
class Tiny_Image {
  ...
  private $compressor = null;
	...
	private function detect_duplicates() {
        ...
        if(is_null($this->sizes[ $size_name ])) continue;
	...
	public function compress_retina( $size_name, $path ) {
      ...
      $preserve = array('copyright', 'creation', 'location');
			try {
				$response = $this->compressor->compress_file( $path, false, $preserve );
				$size->add_tiny_meta( $response );
			} catch (Tiny_Exception $e) {
				$size->add_tiny_meta_error( $e );
			}
    ...
    // Lets include the original size in the mix
    $active_sizes[self::ORIGINAL] = array(
      'width' => 0,
      'height' => 0,
      'tinify' => (isset($active_tinify_sizes[self::ORIGINAL]) &&  $active_tinify_sizes[self::ORIGINAL] == 'Y') ? true : false
    );
					...
					if ( isset( $size->meta['input'] ) ) {
						...
						if ( isset( $size->meta['output'] ) ) {
							...
							if ( $size->modified() ) {
								...
								//if ( in_array( $size_name, $active_tinify_sizes, true ) ) {
                if ( isset($active_tinify_sizes[$size_name]) &&  $active_tinify_sizes[$size_name] == 'Y') {
	...
	public static function get_optimization_statistics($compressor, $settings, $result = null ) {
		global $wpdb;
        ...
        $compressor,
			...
			if ( $image_stats['available_unoptimized_sizes'] > 0 ) {
				$stats['available-for-optimization'][] = $result[ $i ]['ID'];
			}
		}
