		...
		public function template_redirect_access() {
			...
			if ( ( 'sfwd-quiz' === $this->post_type ) && ( 'sfwd-certificates' === $post_type ) ) {
				...
				if ( ! empty( $post ) && is_single() ) {
					...
					if(gettype($cert_post) == 'string') {
						$condition = intval($cert_post) == intval( $post->ID );
					} else {
						$condition = in_array(intval( $post->ID ), $cert_post );
					}

					if ( ! empty( $cert_post ) && $condition == true  ) {

						$current_certificate_id = get_the_ID();
