	...
	class LearnDash_Settings_Metabox_Quiz_Progress_Settings extends LearnDash_Settings_Metabox {
		...
		public function load_settings_values() {
			...
			if ( true === $this->settings_values_loaded ) {
				...
				if ( ( isset( $this->setting_option_values['passingpercentage'] ) ) && ( ! empty( $this->setting_option_values['passingpercentage'] ) ) ) {
				...
				if ( ( isset( $this->setting_option_values['threshold'] ) ) && ( ! empty( $this->setting_option_values['threshold'] ) ) ) {
				...
				if ( $this->quiz_edit ) {
					$this->setting_option_values['timeLimit'] = $this->quiz_edit->getTimeLimit();

					$this->setting_option_values['forcingQuestionSolve'] = $this->quiz_edit->isForcingQuestionSolve();
					...
					if ( ( isset( $this->setting_option_values['repeats'] ) ) && ( '' !== $this->setting_option_values['repeats'] ) ) {
						$this->setting_option_values['quizRunOnceType']   = $this->quiz_edit->getQuizRunOnceType();
						$this->setting_option_values['quizRunOnceCookie'] = $this->quiz_edit->isQuizRunOnceCookie();
					} else {
						$this->setting_option_values['repeats'] = '';
						if ( $this->quiz_edit->isQuizRunOnce() ) {
		...
		public function load_settings_fields() {
			...
			*/
			...
			$this->setting_option_fields = array(
				...
				'timeLimit'               => array(
					'name'           => 'timeLimit',
					'label'          => 'Automatically Submit After',
					'type'           => 'timer-entry',
					'class'          => 'small-text',
					'placeholder'    => 'e.g. 0',
					'default'        => '',
