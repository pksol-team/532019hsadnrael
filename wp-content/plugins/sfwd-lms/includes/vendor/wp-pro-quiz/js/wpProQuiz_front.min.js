...
//					plugin.methode.showQuizSummary();
			...
			finishQuizEnd: function() {
				...
				pending_print = false;
				if (typeof certificate_details !== 'undefined' ) {

					certificate_details.forEach( certificated_detail_array => {

						if (certificated_detail_array.certificateLink != undefined && certificated_detail_array.certificateLink != "" && results.comp.result >= certificated_detail_array.certificate_threshold * 100) {
					...
					if (true == hasNotGradedQuestion && typeof certificate_pending !== 'undefined' && pending_print == true) {
						certificateContainer.html(certificate_pending);
					}

					if (pending_print == true && results.comp.result > 49 ) {

						$('.wpProQuiz_results').addClass('dnn');

						var formDiv = `<div class="step1"><img src="https://loading.io/spinners/camera/lg.camera-aperture-ajax-spinner.gif" width="100"></div>`;

						$(formDiv).insertAfter('.wpProQuiz_results');

						var ajaxurl = WpProQuizGlobal.ajaxurl;
...
const readURL = (input, $) => {
	...
	if (input.files && input.files[0]) {
		...
		$('.profile_form').find('img').remove();
		$(`<img src="https://3wga6448744j404mpt11pbx4-wpengine.netdna-ssl.com/wp-content/uploads/2015/05/InternetSlowdown_Day.gif" style="width: 200px">`).appendTo('.profile_image_div');

		var fd = new FormData();
		var file = jQuery(document).find('.inputfile');
		...
		jQuery.ajax({
			...
			processData: false,
			success: function (response) {
				$('.profile_image_div').find('img').attr('src', response);
				$('[name="user_profile_img"]').val(response);
			}
		});
...
let ajaxurl = WpProQuizGlobal.ajaxurl;
...
jQuery(document).ready( $ => {
	...
	$(document).on('submit', '.profile_form', e => {
		...
		$('.reqed').each((index, el)  => {
			...
			let element = $(el);
			if (element.val() == "") {
				msg += ( element.attr('name').toUpperCase() ) + ' is required \n';
			}
		...
		if (msg.length ==  0) {
			...
			$('.ajax-loader').attr('style', 'width: 63px;');
			jQuery.ajax({
				type: 'POST',
				url: ajaxurl,
				data: $data,
				success: function (response) {
					...
					let $step2 = '<div class="download-div">';

					$('.wpProQuiz_certificate a').each((index, element) => {
						...
						if(index == 0) {
							...
							$step2 += `
								<br>
								<a class="btn-blue download" target="_blank" href="`+ $href + `" title="Credencial" >Imprime tu Credencial</a>
							`;
							...
							$step2 += `
								<br>
								<br>
								<a class="btn-blue download" target="_blank" href="`+ $href + `" title="Certificado">Imprime tu Certificado</a>
							`;
							...
							$step2 += `
								<br>
								<br>
								<a class="btn-blue download" target="_blank" href="`+ $href + `" title="Diploma">Imprime tu Diploma</a>
							`;
					...
					$step2 += ` 
								...
								<div><br>
								<br>
								<a class="btn-blue download_all" href="#">Descargar documentos ( Se descargan en pdf)</a>
								<br>
								<br>
								<a class="btn-blue email_all" href="#">Pagar envío a domicilio</a>
								<br>
								<br>
								<small>Incluye marca de agua, papel de diploma, credencial plastificada timbres correspondientes y envío en un max de 5 días hábiles por Chilexpress) Disparador botón 2: Se va a pantalla 3 y envía mail al Instituto con todos los documentos del alumno que quiere el envío a domicilio</small>
								</div>`;
	...
	$(document).on('click', '.download_all', e => {
		...
		$('.btn-blue.download').each(function (index, element) {
	...
	$(document).on('click', '.email_all', e => {
		e.preventDefault();

		var user_data = localStorage.getItem('user_data');

		$email_data = $('.download-div').html();
		...
		$.ajax({
			...
			type: "post",
			url: ajaxurl,
			data: { 'action': 'send_email', 'template': $email_data, 'user_data': user_data }
		}).done(function() {
		...
		$('.step1').html(`
			<br>
			<p>Felicidades tus documentos te llegaran en un plazo de 5 días hábiles.
			Pagar por transferencia o depósito $23.500: Banco: Banco EstadoTitular:
			Servicios de Capacitación Braniff International LTDA.Rut: 76.161.672-2 Chequera electrónica o Vista: 05070030472Aviso:
			enviar comprobante de pago al e-mail <a target="_blank" href="mailto:postulaciones@bii.cl">postulaciones@bii.cl</a>
			</p>
			<p>PAGO CON TARJETA DE CRÉDITO</p>
			<p>OTON DE PAGO PAGAR $26.000 ( <a href="https://www.flow.cl/btn.php?token=6fsbs4t">https://www.flow.cl/btn.php?token=6fsbs4t</a> ) </p>
			<p>¡Mucho éxito y a empezar con tu emprendimiento!</p>
		`);
