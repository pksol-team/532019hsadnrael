	...
	if ( ( $post instanceof WP_Post ) && ( $post->post_type == 'sfwd-quiz' ) ) {
		...
		if ( is_array( $meta ) && ! empty( $meta ) ) {
			...
			if ( (isset( $meta['sfwd-quiz_certificate'] )) && ( ! empty( $meta['sfwd-quiz_certificate'] ) ) ) {
				...
				if(gettype($certificate_post) == 'array') {

					foreach ($meta['sfwd-quiz_certificate'] as $key => $single_meta) {

						$certificateLink = get_permalink( $certificate_post[$key] );
						...
						if ( ! empty( $certificateLink ) ) {
							...
							$cert_query_args['cert-nonce'] = wp_create_nonce( $post->ID . $cert_user_id . $view_user_id . $certificate_post[$key] );
						...
						array_push($cert_details, array( 'certificateLink' => $certificateLink, 'certificate_threshold' => $certificate_threshold ));
					...
					if ( ! empty( $certificateLink ) ) {
						...
						$cert_query_args['cert-nonce'] = wp_create_nonce( $post->ID . $cert_user_id . $view_user_id . $certificate_post );
...
function ld_course_certificate_shortcode( $atts ) {
	 ...
	 */
	return apply_filters( 'ld_course_certificate', "<div id='learndash_course_certificate'><a href='".$link."' class='btn-blue' target='_blank'>". apply_filters('ld_certificate_link_label', esc_html__( 'PRINT YOUR CERTIFICATEs1', 'learndash' ), $user_id, $course_id ) . '</a></div>', $link, $course_id, $user_id );
}
