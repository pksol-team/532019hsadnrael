...
if ( ( isset( $pager_results ) ) && ( !empty( $pager_results ) ) ) {
	...
	if ( $pager_results['total_pages'] > 1 ) {
		...
		<span><?php echo esc_html($pager_results['paged'] . esc_html__( ' of ', 'learndash' ) . $pager_results['total_pages'] ); ?></span>
			...
			<?php } ?> data-context="<?php echo esc_attr($pager_context); ?>" data-paged="<?php echo $href_val_prefix . $next_page_number . $search_arg; ?>" data-course_id="<?php echo esc_attr($course_id); ?>" <?php echo $data_lesson_id; ?> class="<?php echo $pager_right_class ?>" <?php echo $pager_right_disabled; ?> title="<?php esc_attr_e( 'Next Page', 'leardnash' ); ?>"><span class="ld-icon-arrow-right ld-icon"></span></a>
