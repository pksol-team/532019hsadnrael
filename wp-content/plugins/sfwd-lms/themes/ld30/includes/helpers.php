...
function learndash_get_course_price( $course = null ) {
    ...
    if( $course_price['type'] == 'subscribe' ) {
        ...
        switch( $frequency ) {
            case('D'):
                $label = _n( __('day', 'learndash'), __('days', 'learndsah'), $interval );
                break;
            case('M'):
                $label = _n( __('month', 'learndash'), __('months', 'learndsah'), $interval );
                break;
            case('Y'):
                $label = _n( __('year', 'learndash'), __('years', 'learndsah'), $interval );
                break;
        }
...
function learndash_get_lesson_progress( $topics = null ) {
    ...
    $progress = apply_filters( 'learndash_get_lesson_progress_defaults', array(
        'percentage'    =>  0,
        'completed'     =>  0,
        'total'         =>  0
    ) );
...
function learndash_get_lesson_attributes( $lesson = null ) {
    ...
    if( !empty($lesson['lesson_access_from']) ) {
        $attributes[] = array(
            'label' =>  sprintf( esc_html_x( 'Available on %s', 'Available on date label', 'learndash' ), learndash_adjust_date_time_display( $lesson['lesson_access_from'] ) ),
...
function learndash_get_template_part( $filepath, $args = null, $echo = false ) {
    ...
    /**
     * Temporary until we have a complete way to output partials
     */

     // Normalize the path
     $filepath = ( substr( $filepath, -4 ) !== '.php' ? $filepath . '.php' : $filepath );
     $slug     = str_replace( '.php', '', $filepath );

     // Filter so these templates can be overriden individually
     $filepath = apply_filters( 'learndash_30_get_template_part', LD_30_TEMPLATE_DIR . $filepath, $slug, $args, $echo );
...
function learndash_30_focus_mode( $template ) {
    ...
    if( in_array( get_post_type(), $post_types ) && is_singular($post_types) ) {
...
function learndash_30_template_assets() {
    ...
    wp_register_style( 'learndash-front', LEARNDASH_LMS_PLUGIN_URL . 'themes/ld30/assets/css/learndash.min.css', LD_30_VER );
    wp_register_script( 'learndash-front', LEARNDASH_LMS_PLUGIN_URL . 'themes/ld30/assets/js/learndash.js', array( 'jquery' ), LD_30_VER, true );

    wp_register_style( 'learndash-quiz-front', LEARNDASH_LMS_PLUGIN_URL . 'themes/ld30/assets/css/learndash.quiz.front.min.css', LD_30_VER );
    ...
    wp_localize_script( 'learndash-front', 'ldVars', array(
        'postID' => get_the_ID(),
        'videoReqMsg' => __( 'You must watch the video before accessing this content', 'learndash' )
    ) );
...
function learndash_30_editor_scripts() {
    ...
    wp_enqueue_style( 'learndash-front', LEARNDASH_LMS_PLUGIN_URL . 'themes/ld30/assets/css/learndash.min.css', LD_30_VER );
    wp_enqueue_script( 'learndash-front', LEARNDASH_LMS_PLUGIN_URL . 'themes/ld30/assets/js/learndash.js', array('jquery'), LD_30_VER, true );
...
function learndash_login_shortcode( $atts ) {
    ...
    if( is_user_logged_in() ) {
        $link = apply_filters( 'learndash-login-shortcode-logout', array(
        ...
        $link = apply_filters( 'learndash-login-shortcode-login', array(
            'url'   =>  $login_url,
    ...
    if( get_post_type() !== 'sfwd-courses' && !is_user_logged_in() && $login_model == 'Yes' ) {
...
function learndash_user_status_shortcode( $atts = array() ) {
    ...
    learndash_get_template_part( 'shortcodes/user-status.php', array(
...
class LearnDash_User_Status_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			...
			'description' => sprintf( esc_html_x( 'LearnDash - Registered %s and progress information of users. Visible only to users logged in.', 'placeholders: courses', 'learndash' ), LearnDash_Custom_Label::get_label( 'courses' ) )
		);
	...
	function widget( $args, $instance ) {
        ...
        $user_status = learndash_get_template_part( 'shortcodes/user-status.php', array(
            'course_info'       => $course_info,
            'shortcode_atts'    => $args,
            'context'           => 'widget'
        ), false );
...
function learndash_manage_comments() {
    ...
    $no_comments = apply_filters( 'learndash_no_comments', array(
        'sfwd-courses',
        'sfwd-lessons',
        'sfwd-topic',
        'sfwd-quiz'
    ...
    if( in_array( get_post_type(), $no_comments ) ) {
...
add_action( 'wp_authenticate', 'learndash_login_failed' );
add_action( 'wp_login_failed', 'learndash_login_failed' );
function learndash_login_failed( $user ) {
	...
	if( isset( $_POST[ 'learndash-login-form' ] ) ) {
		...
		if( !empty( $retry ) ) {
			...
			wp_redirect( add_query_arg( 'login', 'failed', $_SERVER['HTTP_REFERER'] ) . '#login' );
		}
...
function learndash_add_login_field( $content ) {

	return $content .= '<input type="hidden" name="learndash-login-form" value="true">';
...
function learndash_focus_mode_comments( $comment, $args, $depth ) {
   ...
   <div <?php comment_class('ld-focus-comment'); ?> id="li-comment-<?php comment_ID() ?>">
       ...
       <div class="ld-comment-avatar">
           ...
           <span class="ld-comment-avatar-author"><span class="ld-comment-author-name"><?php printf(__('%s'), get_comment_author_link()) ?></span> <a class="ld-comment-permalink" href="<?php echo htmlspecialchars ( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf( __( 'on %1$s', 'learndash' ), get_comment_date(), get_comment_time()) ?></a></span>
       </div>
...
function learndash_30_nav_menus() {
    ...
    register_nav_menus( apply_filters( 'learndash_30_nav_menus', array(
...
function learndash_30_ajax_profile_search() {
    ...
    if( !isset($_GET['shortcode_instance']) ) {
        wp_send_json_error( array( 'success' => false, 'message' => __( 'No attributes passed in', 'learndash' ) ) );
    }
...
function learndash_30_ajax_pager() {
    ...
    if( !in_array( $context, $contexts_without_course_id ) && ( !isset($course_id) || empty($course_id) ) ) {
        wp_send_json_error( array( 'success' => false, 'message' => __( 'No course ID supplied', 'learndash' ) ) );
    }
    ...
    if( isset($lesson_id) && !empty($lesson_id) ) {
        ...
        if( empty($topics) || !$topics ) {
            wp_send_json_error( array( 'success' => false, 'message' => __( 'No topics for this lesson', 'learndash' ) ) );
        }
        ...
        wp_send_json_success( array( 'success' => true, 'context' => $context, 'topics' => $topic_list, 'nav_topics' => $nav_topics, 'pager' => $pager, 'lesson_id' => $lesson_id ) );
        ...
        wp_send_json_success( array( 'success' => true, 'context' => $context, 'lessons' => $lesson_list, 'nav_lessons' => $nav_lessons, 'course_id' => $course_id, 'widget_instance' => $widget_instance ) );
    ...
    wp_send_json_error( array( 'success' => false, 'message' => __( 'No Pagination Match', 'learndash' ) ) );
...
add_filter( 'embed_oembed_html', 'learndash_30_responsive_videos', 10, 4 );
function learndash_30_responsive_videos( $html, $url, $attr, $post_id ) {

    $post_types = apply_filters( 'learndash_responsive_video_post_types', array(
        'sfwd-courses',
        'sfwd-lessons',
        'sfwd-topic',
        'sfwd-quiz',
    ...
    $matches = apply_filters( 'learndash_responsive_video_domains', array(
        'youtube.com',
        'vimeo.com'
    ) );
    ...
    foreach( $matches as $match ) {
        if( strpos( $url, $match ) !== false ) {
            return '<div class="ld-video">' . $html . '</div>';
        }
    }
...
function learndash_get_points_awarded_array( $assignment_id ) {
	...
	if ( is_numeric( $current ) ) {
        ...
        return apply_filters( 'learndash_get_points_awarded_array', array(
            'current'   =>  $current,
            'max'       =>  $max_points,
