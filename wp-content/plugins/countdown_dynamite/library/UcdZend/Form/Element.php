...
/** @see UcdZend_Filter */
require_once 'UcdZend/Filter.php';
 ...
 */
class UcdZend_Form_Element implements UcdZend_Validate_Interface
{
    /**
     * Element Constants
     */
     ...
     * - UcdZend_Config: UcdZend_Config with options for configuring element
     *
     * @param  string|array|UcdZend_Config $spec
     * @param  array|UcdZend_Config $options
     * @return void
     * @throws UcdZend_Form_Exception if no element name after initialization
     */
    public function __construct($spec, $options = null)
    {
        ...
        } elseif ($spec instanceof UcdZend_Config) {
            $this->setConfig($spec);
        }

        if (is_string($spec) && is_array($options)) {
            $this->setOptions($options);
        } elseif (is_string($spec) && ($options instanceof UcdZend_Config)) {
            $this->setConfig($options);
        }

        if (null === $this->getName()) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('UcdZend_Form_Element requires each element to have a name');
        }
    ...
    /**
     * Set object state from UcdZend_Config object
     *
     ...
     *
     * @param  UcdZend_Translate|null $translator
     * @return UcdZend_Form_Element
     */
     ...
     *
     * @return UcdZend_Translate_Adapter|null
     */
    public function getTranslator()
    {
        ...
        if (null === $this->_translator) {
            return UcdZend_Form::getDefaultTranslator();
        }
     ...
     * @param  mixed $value
     * @return UcdZend_Form_Element
     * @throws UcdZend_Form_Exception for invalid $name values
     */
    public function setAttrib($name, $value)
    {
        ...
        if ('_' == $name[0]) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Invalid attribute "%s"; must not contain a leading underscore', $name));
        }
    ...
    public function __get($key)
    {
        if ('_' == $key[0]) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Cannot retrieve value for protected/private property "%s"', $key));
        }
    ...
    public function __call($method, $args)
    {
        if ('render' == substr($method, 0, 6)) {
            ...
            require_once 'UcdZend/Form/Element/Exception.php';
            throw new UcdZend_Form_Element_Exception(sprintf('Decorator by name %s does not exist', $decoratorName));
        }

        require_once 'UcdZend/Form/Element/Exception.php';
        throw new UcdZend_Form_Element_Exception(sprintf('Method %s does not exist', $method));
    }
     ...
     */
    public function setPluginLoader(UcdZend_Loader_PluginLoader_Interface $loader, $type)
    {
     ...
     *
     * @param  string $type
     * @return UcdZend_Loader_PluginLoader
     * @throws UcdZend_Loader_Exception on invalid type.
     */
     ...
     *
     * @param  string|UcdZend_Validate_Interface $validator
     * @param  bool $breakChainOnFailure
     * @param  array $options
     * @return UcdZend_Form_Element
     * @throws UcdZend_Form_Exception if invalid validator type
     */
    public function addValidator($validator, $breakChainOnFailure = false, $options = array())
    {
        if ($validator instanceof UcdZend_Validate_Interface) {
            ...
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Invalid validator provided to addValidator; must be string or UcdZend_Validate_Interface');
        }
    ...
    public function addValidators(array $validators)
    {
        foreach ($validators as $validatorInfo) {
            if (is_string($validatorInfo)) {
            ...
            } elseif ($validatorInfo instanceof UcdZend_Validate_Interface) {
                $this->addValidator($validatorInfo);
                ...
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Invalid validator passed to addValidators()');
            }
        }
     ...
     *
     * @param  string $name
     * @return UcdZend_Validate_Interface|false False if not found, validator otherwise
     */
    ...
    public function getValidators()
    {
        ...
        foreach ($this->_validators as $key => $value) {
            if ($value instanceof UcdZend_Validate_Interface) {
    ...
    public function isValid($value, $context = null)
    {
        ...
        // Find the correct translator. UcdZend_Validate_Abstract::getDefaultTranslator()
        // will get either the static translator attached to UcdZend_Validate_Abstract
        // or the 'UcdZend_Translate' from UcdZend_Registry.
        if (UcdZend_Validate_Abstract::hasDefaultTranslator() &&
            !UcdZend_Form::hasDefaultTranslator())
        {
            $translator = UcdZend_Validate_Abstract::getDefaultTranslator();
     ...
     *
     * @param  string|UcdZend_Filter_Interface $filter
     * @return UcdZend_Form_Element
     */
    public function addFilter($filter, $options = array())
    {
        if ($filter instanceof UcdZend_Filter_Interface) {
            ...
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Invalid filter provided to addFilter; must be string or UcdZend_Filter_Interface');
        }
    ...
    public function addFilters(array $filters)
    {
        foreach ($filters as $filterInfo) {
            if (is_string($filterInfo)) {
            ...
            } elseif ($filterInfo instanceof UcdZend_Filter_Interface) {
                $this->addFilter($filterInfo);
                ...
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Invalid filter passed to addFilters()');
            }
        }
     ...
     *
     * @param  string $name
     * @return UcdZend_Filter_Interface
     */
    ...
    public function getFilters()
    {
        ...
        foreach ($this->_filters as $key => $value) {
            if ($value instanceof UcdZend_Filter_Interface) {
                $filters[$key] = $value;
                continue;
            }
     ...
     *
     * @return null|UcdZend_View_Interface
     */
    ...
    protected function _loadFilter(array $filter)
    {
        ...
        if (array_key_exists($name, $this->_filters)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Filter instance already exists for filter "%s"', $origName));
        }
     ...
     * @return UcdZend_Validate_Interface
     */
    protected function _loadValidator(array $validator)
    {
        ...
        if (array_key_exists($name, $this->_validators)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Validator instance already exists for validator "%s"', $origName));
        }
