...
/**
 * UcdZend_Form_Decorator_Form
 *
 * Render a UcdZend_Form object.
 *
 ...
 */
class UcdZend_Form_Decorator_Form extends UcdZend_Form_Decorator_Abstract
{
    /**
     * Default view helper
     * @var string
     ...
     *
     * @param  string $helper
     * @return UcdZend_Form_Decorator_Form
     */
    ...
    public function getOptions()
    {
        if (null !== ($element = $this->getElement())) {
            if ($element instanceof UcdZend_Form) {
                $element->getAction();
                $method = $element->getMethod();
                if ($method == UcdZend_Form::METHOD_POST) {
                    $this->setOption('enctype', 'application/x-www-form-urlencoded');
                }
                foreach ($element->getAttribs() as $key => $value) {
                    $this->setOption($key, $value);
                }
            } elseif ($element instanceof UcdZend_Form_DisplayGroup) {
                foreach ($element->getAttribs() as $key => $value) {
                    $this->setOption($key, $value);
                }
            }
