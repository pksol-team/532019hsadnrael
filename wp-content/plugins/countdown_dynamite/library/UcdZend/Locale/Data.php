...
class UcdZend_Locale_Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new UcdZend_Locale_Exception("Missing locale file '$filename' for '$locale' locale.");
            }
     ...
     *
     * @param string|UcdZend_Locale $locale Locale to check
     * @return string
     */
    private static function _checkLocale($locale)
    {
        ...
        if (!(UcdZend_Locale::isLocale((string) $locale, null, false))) {
            require_once 'UcdZend/Locale/Exception.php';
            throw new UcdZend_Locale_Exception("Locale (" . (string) $locale . ") is a unknown locale");
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            require_once 'UcdZend/Cache.php';
            self::$_cache = UcdZend_Cache::factory(
                'Core',
                'File',
        ...
        $val = urlencode($val);
        $id = strtr('UcdZend_LocaleL_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'UcdZend/Locale/Exception.php';
                throw new UcdZend_Locale_Exception("Unknown list ($path) for parsing locale data.");
                break;
        }
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        $val = urlencode($val);
        $id = strtr('UcdZend_LocaleC_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        if (!self::$_cacheDisabled && ($result = self::$_cache->load($id))) {
            return unserialize($result);
        }
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'UcdZend/Locale/Exception.php';
                throw new UcdZend_Locale_Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
    ...
    /**
     * Set a cache for UcdZend_Locale_Data
     *
     * @param UcdZend_Cache_Core $cache A cache frontend
     */
