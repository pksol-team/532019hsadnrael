 ...
 * @see UcdZend_Config
 */
require_once 'UcdZend/Config.php';

/**
 * XML Adapter for UcdZend_Config
 *
 * @category  Zend
 * @package   UcdZend_Config
 * @copyright  Copyright (c) 2005-2010 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Config_Xml extends UcdZend_Config
{
    /**
     ...
     * @throws UcdZend_Config_Exception When xml is not set or cannot be loaded
     * @throws UcdZend_Config_Exception When section $sectionName cannot be found in $xml
     */
    public function __construct($xml, $section = null, $options = false)
    {
        if (empty($xml)) {
            require_once 'UcdZend/Config/Exception.php';
            throw new UcdZend_Config_Exception('Filename is not set');
        }
        ...
        if ($this->_loadFileErrorStr !== null) {
            require_once 'UcdZend/Config/Exception.php';
            throw new UcdZend_Config_Exception($this->_loadFileErrorStr);
        }
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config->$sectionName)) {
                    require_once 'UcdZend/Config/Exception.php';
                    throw new UcdZend_Config_Exception("Section '$sectionName' cannot be found in $xml");
                }
            ...
            if (!isset($config->$section)) {
                require_once 'UcdZend/Config/Exception.php';
                throw new UcdZend_Config_Exception("Section '$section' cannot be found in $xml");
            }
     ...
     * @throws UcdZend_Config_Exception When $section cannot be found
     * @return array
     */
    protected function _processExtends(SimpleXMLElement $element, $section, array $config = array())
    {
        if (!isset($element->$section)) {
            require_once 'UcdZend/Config/Exception.php';
            throw new UcdZend_Config_Exception("Section '$section' cannot be found");
        }
    ...
    protected function _toArray(SimpleXMLElement $xmlObject)
    {
        ...
        if (count($xmlObject->children(self::XML_NAMESPACE)) > 0) {
            if (count($xmlObject->children()) > 0) {
                require_once 'UcdZend/Config/Exception.php';
                throw new UcdZend_Config_Exception("A node with a 'const' childnode may not have any other children");
            }
                ...
                switch ($node->localName) {
                    case 'const':
                        if (!$node->hasAttributeNS(self::XML_NAMESPACE, 'name')) {
                            require_once 'UcdZend/Config/Exception.php';
                            throw new UcdZend_Config_Exception("Misssing 'name' attribute in 'const' node");
                        }
                        ...
                        if (!defined($constantName)) {
                            require_once 'UcdZend/Config/Exception.php';
                            throw new UcdZend_Config_Exception("Constant with name '$constantName' was not defined");
                        }
                    ...
                    default:
                        require_once 'UcdZend/Config/Exception.php';
                        throw new UcdZend_Config_Exception("Unknown node with name '$node->localName' found");
                }
            }
