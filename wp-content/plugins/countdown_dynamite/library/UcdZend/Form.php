...
/** @see UcdZend_Validate_Interface */
require_once 'UcdZend/Validate/Interface.php';
 ...
 */
class UcdZend_Form implements Iterator, Countable, UcdZend_Validate_Interface
{
    /**#@+
     ...
     * @var string
     */
    protected $_defaultDisplayGroupClass = 'UcdZend_Form_DisplayGroup';
    ...
    public function __construct($options = null)
    {
        ...
        } elseif ($options instanceof UcdZend_Config) {
     ...
     *
     * @param  UcdZend_Loader_PluginLoader_Interface $loader
     * @param  string $type 'decorator' or 'element'
     * @return UcdZend_Form
     * @throws UcdZend_Form_Exception on invalid type
     */
    public function setPluginLoader(UcdZend_Loader_PluginLoader_Interface $loader, $type = null)
    {
        ...
        switch ($type) {
            ...
            case self::ELEMENT:
                ...
                return $this;
            default:
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    }
     ...
     *
     * @param  string $type
     * @return UcdZend_Loader_PluginLoader_Interface
     */
    public function getPluginLoader($type = null)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                ...
                default:
                    require_once 'UcdZend/Form/Exception.php';
                    throw new UcdZend_Form_Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }

            require_once 'UcdZend/Loader/PluginLoader.php';
            $this->_loaders[$type] = new UcdZend_Loader_PluginLoader(
                array('UcdZend_' . $prefixSegment . '_' => 'UcdZend/' . $pathSegment . '/')
            );
        }
     ...
     * @param  string $type
     * @return UcdZend_Form
     * @throws UcdZend_Form_Exception for invalid type
     */
    ...
    public function setMethod($method)
    {
        ...
        if (!in_array($method, $this->_methods)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('"%s" is an invalid form method', $method));
        }
    ...
    public function setName($name)
    {
        ...
        if ('' === (string)$name) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Invalid name provided; must contain only valid variable characters and be non-empty');
        }
     ...
     * UcdZend_Form_Element. If a string element type is provided, $name must be
     * provided, and $options may be optionally provided for configuring the
     * element.
     *
     * If a UcdZend_Form_Element is provided, $name may be optionally provided,
     * and any provided $options will be ignored.
     *
     * @param  string|UcdZend_Form_Element $element
     * @param  string $name
     * @param  array|UcdZend_Config $options
     * @return UcdZend_Form
     */
    public function addElement($element, $name = null, $options = null)
    {
        if (is_string($element)) {
            if (null === $name) {
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Elements specified by string must have an accompanying name');
            }
        ...
        } elseif ($element instanceof UcdZend_Form_Element) {
            $prefixPaths              = array();
     ...
     * @param  string $name
     * @param  array|UcdZend_Config $options
     * @return UcdZend_Form_Element
     */
    public function createElement($type, $name, $options = null)
    {
        if (!is_string($type)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Element type must be a string indicating type');
        }

        if (!is_string($name)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Element name must be a string');
        }
    ...
    public function addElements(array $elements)
    {
        foreach ($elements as $key => $spec) {
            ...
            if (is_string($spec) || ($spec instanceof UcdZend_Form_Element)) {
     ...
     *
     * @param  string $name
     * @return UcdZend_Form_Element|null
     */
     ...
     *
     * @param  UcdZend_Form $form
     * @param  string $name
     * @param  int $order
     * @return UcdZend_Form
     */
    public function addSubForm(UcdZend_Form $form, $name, $order = null)
    {
        $name = (string) $name;
    ...
    public function addSubForms(array $subForms)
    {
        foreach ($subForms as $key => $spec) {
            ...
            if ($spec instanceof UcdZend_Form) {
     ...
     * @param  array|UcdZend_Config $options
     * @return UcdZend_Form
     * @throws UcdZend_Form_Exception if no valid elements provided
     */
    public function addDisplayGroup(array $elements, $name, $options = null)
    {
        ...
        if (empty($group)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('No valid elements specified for display group');
        }
        ...
        if (!class_exists($class)) {
            ...
            UcdZend_Loader::loadClass($class);
        }
     ...
     *
     * @param  UcdZend_Form_DisplayGroup $group
     * @param  string|null $name
     * @return UcdZend_Form
     */
    protected function _addDisplayGroupObject(UcdZend_Form_DisplayGroup $group, $name = null)
    {
        if (null === $name) {
            $name = $group->getName();
            if ('' === (string)$name) {
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Invalid display group added; requires name');
            }
        }
    ...
    public function addDisplayGroups(array $groups)
    {
        foreach ($groups as $key => $spec) {
            ...
            if ($spec instanceof UcdZend_Form_DisplayGroup) {
     ...
     *
     * @param  string $name
     * @return UcdZend_Form_DisplayGroup|null
     */
    ...
    public function getElementsAndSubFormsOrdered()
    {
        ...
        foreach ($this->_order as $name => $order) {
            ...
            if ($this->$name instanceof UcdZend_Form_Element ||
                $this->$name instanceof UcdZend_Form) {
                array_splice($ordered, $order, 0, array($this->$name));
            } else if ($this->$name instanceof UcdZend_Form_DisplayGroup) {
                $subordered = array();
    ...
    public function isValid($data)
    {
        if (!is_array($data)) {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(__METHOD__ . ' expects an array');
        }
    ...
    {
        require_once 'UcdZend/Json.php';
        if ($this->isValidPartial($data)) {
            return UcdZend_Json::encode(true);
        }
        $messages = $this->getMessages();
        return UcdZend_Json::encode($messages);
    }
     ...
     * Set view object
     *
     * @param  UcdZend_View_Interface $view
     * @return UcdZend_Form
     */
    public function setView(UcdZend_View_Interface $view = null)
    {
        $this->_view = $view;
        return $this;
    }
     ...
     *
     * @return UcdZend_View_Interface|null
     */
    public function getView()
    {
        if (null === $this->_view) {
            require_once 'UcdZend/Controller/Action/HelperBroker.php';
            $viewRenderer = UcdZend_Controller_Action_HelperBroker::getStaticHelper('viewRenderer');
     ...
     * @return UcdZend_Form_Decorator_Interface
     */
     ...
     *
     * @param  string|UcdZend_Form_Decorator_Interface $decorator
     * @param  array|UcdZend_Config $options Options with which to initialize decorator
     * @return UcdZend_Form
     */
    public function addDecorator($decorator, $options = null)
    {
        if ($decorator instanceof UcdZend_Form_Decorator_Interface) {
            ...
            if (is_numeric($name)) {
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Invalid alias provided to addDecorator; must be alphanumeric string');
            }
            if (is_string($spec)) {
                $decorator = array(
                    'decorator' => $spec,
            ...
            } elseif ($spec instanceof UcdZend_Form_Decorator_Interface) {
                $decorator = $spec;
            }
        } else {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Invalid decorator provided to addDecorator; must be string or UcdZend_Form_Decorator_Interface');
        }
    ...
    public function addDecorators(array $decorators)
    {
        foreach ($decorators as $decoratorName => $decoratorInfo) {
            if (is_string($decoratorInfo) ||
                $decoratorInfo instanceof UcdZend_Form_Decorator_Interface) {
                ...
                require_once 'UcdZend/Form/Exception.php';
                throw new UcdZend_Form_Exception('Invalid decorator passed to addDecorators()');
            }
        }
     ...
     *
     * @param  string $name
     * @return false|UcdZend_Form_Decorator_Abstract
     */
     ...
     * @return string
     */
    public function render(UcdZend_View_Interface $view = null)
    {
        if (null !== $view) {
            $this->setView($view);
        }
     ...
     *
     * @param  UcdZend_Translate|UcdZend_Translate_Adapter|null $translator
     * @return UcdZend_Form
     */
    public function setTranslator($translator = null)
    {
        ...
        } elseif ($translator instanceof UcdZend_Translate_Adapter) {
            $this->_translator = $translator;
        } elseif ($translator instanceof UcdZend_Translate) {
            $this->_translator = $translator->getAdapter();
        } else {
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception('Invalid translator specified');
        }
     ...
     *
     * @return null|UcdZend_Translate
     */
    public static function getDefaultTranslator()
    {
        if (null === self::$_translatorDefault) {
            require_once 'UcdZend/Registry.php';
            if (UcdZend_Registry::isRegistered('UcdZend_Translate')) {
                $translator = UcdZend_Registry::get('UcdZend_Translate');
                if ($translator instanceof UcdZend_Translate_Adapter) {
                    return $translator;
                } elseif ($translator instanceof UcdZend_Translate) {
     ...
     *
     * @param  string $name
     * @return UcdZend_Form_Element|UcdZend_Form|null
     */
     ...
     *
     * @param  string $name
     * @param  UcdZend_Form_Element|UcdZend_Form $value
     * @return void
     * @throws UcdZend_Form_Exception for invalid $value
     */
    public function __set($name, $value)
    {
        if ($value instanceof UcdZend_Form_Element) {
            $this->addElement($value, $name);
            return;
        } elseif ($value instanceof UcdZend_Form) {
        ...
        throw new UcdZend_Form_Exception('Only form elements and groups may be overloaded; variable of type "' . $type . '" provided');
    }
     ...
     * @param  array $args
     * @return string
     * @throws UcdZend_Form_Exception for invalid decorator or invalid method call
     */
    public function __call($method, $args)
    {
        if ('render' == substr($method, 0, 6)) {
            ...
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Decorator by name %s does not exist', $decoratorName));
        }

        require_once 'UcdZend/Form/Exception.php';
        throw new UcdZend_Form_Exception(sprintf('Method %s does not exist', $method));
    }
     ...
     *
     * @return UcdZend_Form_Element|UcdZend_Form_DisplayGroup|UcdZend_Form
     */
    public function current()
    {
        ...
        if (isset($this->_elements[$key])) {
            ...
            require_once 'UcdZend/Form/Exception.php';
            throw new UcdZend_Form_Exception(sprintf('Corruption detected in form; invalid key ("%s") found in internal iterator', (string) $key));
        }
    }
    ...
    public function loadDefaultDecorators()
    {
        ...
        if (empty($decorators)) {
            $this->addDecorator('FormElements')
                 ->addDecorator('HtmlTag', array('tag' => 'dl', 'class' => 'UcdZend_form'))
                 ->addDecorator('Form');
        }
        return $this;
    }
