 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class UcdZend_File_Transfer_Adapter_Abstract
{
    /**@+
    ...
    {
        require_once 'UcdZend/File/Transfer/Exception.php';
     ...
     * @return UcdZend_File_Transfer_Adapter_Abstract
     * @throws UcdZend_File_Transfer_Exception on invalid type
     */
    public function setPluginLoader(UcdZend_Loader_PluginLoader_Interface $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    }
        ...
        switch ($type) {
            ...
            case self::VALIDATE:
                ...
                if (!isset($this->_loaders[$type])) {
                    $paths         = array(
                        'UcdZend_' . $prefixSegment . '_'     => 'UcdZend/' . $pathSegment . '/',
                        'UcdZend_' . $prefixSegment . '_File' => 'UcdZend/' . $pathSegment . '/File',
                    );

                    require_once 'UcdZend/Loader/PluginLoader.php';
                    $this->_loaders[$type] = new UcdZend_Loader_PluginLoader($paths);
                } else {
                    $loader = $this->_loaders[$type];
                    $prefix = 'UcdZend_' . $prefixSegment . '_File_';
            ...
            default:
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
        }
    }
     ...
     * @param  string $type
     * @return UcdZend_File_Transfer_Adapter_Abstract
     * @throws UcdZend_File_Transfer_Exception for invalid type
     */
     ...
     *
     * @param  array $spec
     * @return UcdZend_File_Transfer_Exception
     */
     ...
     * @return UcdZend_File_Transfer_Adapter
     */
    public function addValidator($validator, $breakChainOnFailure = false, $options = null, $files = null)
    {
        if ($validator instanceof UcdZend_Validate_Interface) {
            ...
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('Invalid validator provided to addValidator; must be string or UcdZend_Validate_Interface');
        }
    ...
    public function addValidators(array $validators, $files = null)
    {
        foreach ($validators as $name => $validatorInfo) {
            if ($validatorInfo instanceof UcdZend_Validate_Interface) {
                ...
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception('Invalid validator passed to addValidators()');
            }
        }
     ...
     *
     * @param  string $name
     * @return UcdZend_Validate_Interface|null
     */
            ...
            foreach ($options as $name => $value) {
                foreach ($file as $key => $content) {
                    switch ($name) {
                        ...
                        default:
                            require_once 'UcdZend/File/Transfer/Exception.php';
                            throw new UcdZend_File_Transfer_Exception("Unknown option: $name = $value");
                    }
                }
            }
        }
    ...
    public function isValid($files = null)
    {
        ...
        foreach($check as $key => $content) {
            if (array_key_exists('validators', $content) &&
                in_array('UcdZend_Validate_File_Count', $content['validators'])) {
                $validator = $this->_validators['UcdZend_Validate_File_Count'];
                $count     = $content;
        ...
        foreach ($check as $key => $content) {
            ...
            if (array_key_exists('validators', $content)) {
                foreach ($content['validators'] as $class) {
                    ...
                    if (($class === 'UcdZend_Validate_File_Upload') and (empty($content['tmp_name']))) {
                        $tocheck = $key;
                    } else {
                    ...
                    if (($class === 'UcdZend_Validate_File_Upload') and (count($fileerrors) > 0)) {
                        break;
                    }
    ...
    public function addFilter($filter, $options = null, $files = null)
    {
        if ($filter instanceof UcdZend_Filter_Interface) {
            ...
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('Invalid filter specified');
        }
    ...
    public function addFilters(array $filters, $files = null)
    {
        foreach ($filters as $key => $spec) {
            if ($spec instanceof UcdZend_Filter_Interface) {
     ...
     *
     * @param  string $name
     * @return UcdZend_Filter_Interface|null
     */
     ...
     * @throws UcdZend_File_Transfer_Exception When file not found
     */
     ...
     * @throws UcdZend_File_Transfer_Exception Not implemented
     */
    public function getFile()
    {
        require_once 'UcdZend/File/Transfer/Exception.php';
        throw new UcdZend_File_Transfer_Exception('Method not implemented');
    }
     ...
     * @return UcdZend_File_Transfer_Abstract
     * @throws UcdZend_File_Transfer_Exception when the given destination is not a directory or does not exist
     */
    public function setDestination($destination, $files = null)
    {
        ...
        if (!is_dir($destination)) {
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('The given destination is not a directory or does not exist');
        }

        if (!is_writable($destination)) {
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('The given destination is not writeable');
        }
    ...
    public function getDestination($files = null)
    {
        ...
        if (empty($files) and is_string($orig)) {
            if (isset($this->_files[$orig]['destination'])) {
                ...
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception(sprintf('The file transfer adapter can not find "%s"', $orig));
            }
        }
    ...
    public function setTranslator($translator = null)
    {
        if (null === $translator) {
            ...
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('Invalid translator specified');
        }
     ...
     * @throws UcdZend_File_Transfer_Exception On unknown hash algorithm
     */
    public function getHash($hash = 'crc32', $files = null)
    {
        if (!in_array($hash, hash_algos())) {
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('Unknown hash algorithm');
        }
        ...
        foreach($files as $key => $value) {
            if (file_exists($value['name'])) {
                ...
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception("The file '{$value['name']}' does not exist");
            }
        }
     ...
     * @throws UcdZend_File_Transfer_Exception When the file does not exist
        ...
        foreach ($check as $name => $content) {
            if (array_key_exists('filters', $content)) {
                foreach ($content['filters'] as $class) {
                    ...
                    } catch (UcdZend_Filter_Exception $e) {
     ...
     *
     * @return string
     * @throws UcdZend_File_Transfer_Exception if unable to determine directory
     */
        ...
        if (null === $this->_tmpDir) {
            ...
            if (empty($this->_tmpDir)) {
                ...
                if ($tempFile) {
                    ...
                    require_once 'UcdZend/File/Transfer/Exception.php';
                    throw new UcdZend_File_Transfer_Exception('Could not determine a temporary directory');
                }
            }
     ...
     * @throws UcdZend_File_Transfer_Exception On false filename
     */
        ...
        if (is_array($files)) {
            foreach ($files as $find) {
                ...
                if (empty($found)) {
                    ...
                    require_once 'UcdZend/File/Transfer/Exception.php';
                    throw new UcdZend_File_Transfer_Exception(sprintf('The file transfer adapter can not find "%s"', $find));
                }
