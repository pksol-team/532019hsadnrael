...
/**
 * @see UcdZend_File_Transfer_Adapter_Abstract
 */
require_once 'UcdZend/File/Transfer/Adapter/Abstract.php';
 ...
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_File_Transfer_Adapter_Http extends UcdZend_File_Transfer_Adapter_Abstract
{
    ...
    public function __construct($options = array())
    {
        if (ini_get('file_uploads') == false) {
            ...
            throw new UcdZend_File_Transfer_Exception('File uploads are not allowed in your php config!');
        }
    ...
    public function isValid($files = null)
    {
        ...
        if (empty($this->_files) && ($content > 0)) {
            ...
            $temp = array($files => array(
                'name'  => $files,
                'error' => 1));
            $validator = $this->_validators['UcdZend_Validate_File_Upload'];
    ...
    public static function getProgress($id = null)
    {
        if (!function_exists('apc_fetch') and !function_exists('uploadprogress_get_info')) {
            require_once 'UcdZend/File/Transfer/Exception.php';
            throw new UcdZend_File_Transfer_Exception('Neither APC nor uploadprogress extension installed');
        }

        $session = 'UcdZend_File_Transfer_Adapter_Http_ProgressBar';
        $status  = array(
            'total'    => 0,
            'current'  => 0,
            'rate'     => 0,
        ...
        if (!empty($id) && (($id instanceof UcdZend_ProgressBar_Adapter) || ($id instanceof UcdZend_ProgressBar))) {
            $adapter = $id;
            unset($id);
        }
        ...
        if (isset($adapter) && isset($status['id'])) {
            if ($adapter instanceof UcdZend_ProgressBar_Adapter) {
                require_once 'UcdZend/ProgressBar.php';
                $adapter = new UcdZend_ProgressBar($adapter, 0, $status['total'], $session);
            }

            if (!($adapter instanceof UcdZend_ProgressBar)) {
                require_once 'UcdZend/File/Transfer/Exception.php';
                throw new UcdZend_File_Transfer_Exception('Unknown Adapter given');
            }
