...
/**
 * @see UcdZend_Http_Client_Adapter_Interface
 */
require_once 'UcdZend/Http/Client/Adapter/Interface.php';


/**
 * @see UcdZend_Http_Response
 */
require_once 'UcdZend/Http/Response.php';

/**
 * @see UcdZend_Http_Response_Stream
 */
require_once 'UcdZend/Http/Response/Stream.php';

/**
 * UcdZend_Http_Client is an implemetation of an HTTP client in PHP. The client
 * supports basic features like sending different HTTP requests and handling
 * redirections, as well as more advanced features like proxy settings, HTTP
 * authentication and cookie persistance (using a UcdZend_Http_CookieJar object)
 *
 * @todo Implement proxy settings
 * @category   Zend
 * @package    UcdZend_Http
 * @subpackage Client
 * @throws     UcdZend_Http_Client_Exception
 * @copyright  Copyright (c) 2005-2010 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Http_Client
{
    /**
    ...
    protected $config = array(
        ...
        'useragent'       => 'UcdZend_Http_Client',
        'timeout'         => 10,
        'adapter'         => 'UcdZend_Http_Client_Adapter_Socket',
     ...
     *
     * @var UcdZend_Http_Client_Adapter_Interface
     */
     ...
     *
     * @param UcdZend_Uri_Http|string $uri
     ...
     *
     * @param  UcdZend_Uri_Http|string $uri
     * @return UcdZend_Http_Client
     * @throws UcdZend_Http_Client_Exception
     */
    public function setUri($uri)
    {
        if (is_string($uri)) {
            $uri = UcdZend_Uri::factory($uri);
        }

        if (!$uri instanceof UcdZend_Uri_Http) {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception('Passed parameter is not a valid HTTP URI.');
        }
     ...
     * @return UcdZend_Uri_Http|string
     */
    public function getUri($as_string = false)
    {
        if ($as_string && $this->uri instanceof UcdZend_Uri_Http) {
     ...
     *
     * @param  UcdZend_Config | array $config
     * @return UcdZend_Http_Client
     * @throws UcdZend_Http_Client_Exception
     */
    public function setConfig($config = array())
    {
        if ($config instanceof UcdZend_Config) {
            $config = $config->toArray();

        } elseif (! is_array($config)) {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception('Array or UcdZend_Config object expected, got ' . gettype($config));
        }
        ...
        if ($this->adapter instanceof UcdZend_Http_Client_Adapter_Interface) {
    ...
    public function setMethod($method = self::GET)
    {
        if (! preg_match('/^[^\x00-\x1f\x7f-\xff\(\)<>@,;:\\\\"\/\[\]\?={}\s]+$/', $method)) {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception("'{$method}' is not a valid HTTP request method.");
        }
    ...
    public function setHeaders($name, $value = null)
    {
        ...
        if (is_array($name)) {
            ...
            if ($this->config['strict'] && (! preg_match('/^[a-zA-Z0-9-]+$/', $name))) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("{$name} is not a valid HTTP header name");
            }
     ...
     * <code>
     * $this->setAuth('shahar', 'secret', UcdZend_Http_Client::AUTH_BASIC);
     * </code>
     *
    ...
    public function setAuth($user, $password = '', $type = self::AUTH_BASIC)
    {
        ...
        if ($user === false || $user === null) {
            ...
            if ($this->uri instanceof UcdZend_Uri_Http) {
        ...
        // Else, set up authentication
            ...
            if (! defined('self::AUTH_' . strtoupper($type))) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("Invalid or not supported authentication type: '$type'");
            }
     ...
     * and responses.
     *
     * @param UcdZend_Http_CookieJar|boolean $cookiejar Existing cookiejar object, true to create a new one, false to disable
     * @return UcdZend_Http_Client
     * @throws UcdZend_Http_Client_Exception
     */
    public function setCookieJar($cookiejar = true)
    {
        UcdZend_Loader::loadClass('UcdZend_Http_CookieJar');

        if ($cookiejar instanceof UcdZend_Http_CookieJar) {
            $this->cookiejar = $cookiejar;
        } elseif ($cookiejar === true) {
            $this->cookiejar = new UcdZend_Http_CookieJar();
        } elseif (! $cookiejar) {
            $this->cookiejar = null;
        } else {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception('Invalid parameter type passed as CookieJar');
        }
     ...
     *
     * @return UcdZend_Http_CookieJar|null
     */
     ...
     *
     * @param UcdZend_Http_Cookie|string $cookie
    ...
    {
        UcdZend_Loader::loadClass('UcdZend_Http_Cookie');
        ...
        if (isset($this->cookiejar)) {
            if ($cookie instanceof UcdZend_Http_Cookie) {
                $this->cookiejar->addCookie($cookie);
            } elseif (is_string($cookie) && $value !== null) {
                $cookie = UcdZend_Http_Cookie::fromString("{$cookie}={$value}",
                                                       $this->uri,
            ...
            if (preg_match("/[=,; \t\r\n\013\014]/", $cookie)) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("Cookie name cannot contain these characters: =,; \t\r\n\013\014 ({$cookie})");
            }
    ...
    public function setFileUpload($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            if (($data = @file_get_contents($filename)) === false) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("Unable to read file '{$filename}' for upload");
            }
     ...
     *
     * @return UcdZend_Http_Response or null if none
     */
     ...
     *
     * @param UcdZend_Http_Client_Adapter_Interface|string $adapter
     * @return null
     * @throws UcdZend_Http_Client_Exception
     */
    public function setAdapter($adapter)
    {
        if (is_string($adapter)) {
            ...
            } catch (UcdZend_Exception $e) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("Unable to load adapter '$adapter': {$e->getMessage()}", 0, $e);
            }

            $adapter = new $adapter;
        }

        if (! $adapter instanceof UcdZend_Http_Client_Adapter_Interface) {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception('Passed adapter is not a HTTP connection adapter');
        }
     ...
     * Load the connection adapter
     *
     * @return UcdZend_Http_Client_Adapter_Interface $adapter
     */
     ...
     * @return resource
     */
    protected function _openTempStream($path = NULL)
    {
        $this->_stream_name = $this->config['output_stream'];
        if(!is_string($this->_stream_name)) {
            // If name is not given, create temp name
            if (is_null($path)) {
                $path = isset($this->config['stream_tmp_dir'])
                    ? $this->config['stream_tmp_dir']
                    : sys_get_temp_dir();
            }
            $this->_stream_name = @tempnam($path, 'UcdZend_Http_Client');
        }

        if (false === ($fp = @fopen($this->_stream_name, "w+b"))) {
                if (is_null($path) && isset($this->config['fallback_stream_tmp_dir'])) { 
                    return $this->_openTempStream($this->config['fallback_stream_tmp_dir']);
                }
            
                // CB Goliath 3.2.1: go w/o stream if can't open
                ...
                //require_once 'UcdZend/Http/Client/Exception.php';
                //throw new UcdZend_Http_Client_Exception("Could not open temp file {$this->_stream_name}");
        }
    ...
    {     
        if (! $this->uri instanceof UcdZend_Uri_Http) {
            /** @see UcdZend_Http_Client_Exception */
            require_once 'UcdZend/Http/Client/Exception.php';
            throw new UcdZend_Http_Client_Exception('No valid URI has been passed to the client');
        }
        ...
        do {
            ...
            if(is_resource($body) && !($this->adapter instanceof UcdZend_Http_Client_Adapter_Stream)) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception('Adapter does not support streaming');
            }
            ...
            if($this->config['output_stream']) {
                if($this->adapter instanceof UcdZend_Http_Client_Adapter_Stream) {
                    // CB Goliath 3.2.1: go w/o stream if can't open
                    if ($stream = $this->_openTempStream()) {
                        $this->adapter->setOutputStream($stream);
                    } else {
                        $this->setConfig(array('output_stream' => NULL));
                    }
                } else {
                    /** @see UcdZend_Http_Client_Exception */
                    require_once 'UcdZend/Http/Client/Exception.php';
            ...
            if (! $response) {
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception('Unable to read response, or response is empty');
            }
            ...
            if($this->config['output_stream']) {
                ...
                $response = UcdZend_Http_Response_Stream::fromStream($response, $stream);
                ...
                $response = UcdZend_Http_Response::fromString($response);
            }
            ...
            if ($response->isRedirect() && ($location = $response->getHeader('location'))) {
                ...
                if (UcdZend_Uri_Http::check($location)) {
    ...
    protected function _prepareHeaders()
    {
        ...
        if (isset($this->cookiejar)) {
            $cookstr = $this->cookiejar->getMatchingCookies($this->uri,
                true, UcdZend_Http_CookieJar::COOKIE_STRING_CONCAT);
        ...
        if (count($this->paramsPost) > 0 || count($this->files) > 0) {
            switch($this->enctype) {
                ...
                default:
                    ...
                    /** @see UcdZend_Http_Client_Exception */
                    require_once 'UcdZend/Http/Client/Exception.php';
                    throw new UcdZend_Http_Client_Exception("Cannot handle content type '{$this->enctype}' automatically." .
                        " Please use UcdZend_Http_Client::setRawData to send this kind of content.");
                    break;
            }
        }
        ...
        switch ($type) {
            case self::AUTH_BASIC:
                ...
                if (strpos($user, ':') !== false) {
                    /** @see UcdZend_Http_Client_Exception */
                    require_once 'UcdZend/Http/Client/Exception.php';
                    throw new UcdZend_Http_Client_Exception("The user name cannot contain ':' in 'Basic' HTTP authentication");
                }
            ...
            default:
                /** @see UcdZend_Http_Client_Exception */
                require_once 'UcdZend/Http/Client/Exception.php';
                throw new UcdZend_Http_Client_Exception("Not a supported HTTP authentication type: '$type'");
        }
