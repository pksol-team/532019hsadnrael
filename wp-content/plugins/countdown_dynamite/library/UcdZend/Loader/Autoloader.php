 ...
 *
 * @uses       UcdZend_Loader_Autoloader
 * @package    UcdZend_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2010 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Loader_Autoloader
{
    /**
     * @var UcdZend_Loader_Autoloader Singleton instance
     */
     ...
     */
    protected $_defaultAutoloader = array('UcdZend_Loader', 'loadClass');
     ...
     *
     * @return UcdZend_Loader_Autoloader
     */
    ...
    public static function autoload($class)
    {
        ...
        foreach ($self->getClassAutoloaders($class) as $autoloader) {
            if ($autoloader instanceof UcdZend_Loader_Autoloader_Interface) {
    ...
    public function setDefaultAutoloader($callback)
    {
        if (!is_callable($callback)) {
            throw new UcdZend_Loader_Exception('Invalid callback specified for default autoloader');
        }
     ...
     *
     * @param  array $autoloaders Array of PHP callbacks (or UcdZend_Loader_Autoloader_Interface implementations) to act as autoloaders
     * @return UcdZend_Loader_Autoloader
     */
    ...
    public function registerNamespace($namespace)
    {
        if (is_string($namespace)) {
            ...
            throw new UcdZend_Loader_Exception('Invalid namespace provided');
        }
    ...
    public function setZfPath($spec, $version = 'latest')
    {
        ...
        if (is_array($spec)) {
            if (!isset($spec['path'])) {
                throw new UcdZend_Loader_Exception('No path specified for ZF');
            }
            $path = $spec['path'];
     ...
     * @return bool|UcdZend_Loader_Autoloader Returns boolean if no argument is passed, object instance otherwise
     */
     ...
     *
     * @param  object|array|string $callback PHP callback or UcdZend_Loader_Autoloader_Interface implementation
    ...
    protected function _getVersionPath($path, $version)
    {
        ...
        if (empty($availableVersions)) {
            throw new UcdZend_Loader_Exception('No valid ZF installations discovered');
        }
     ...
     * @throws UcdZend_Loader_Exception if version string contains too many dots
     */
    protected function _getVersionType($version)
    {
        ...
        if (3 < $count) {
            throw new UcdZend_Loader_Exception('Invalid version string provided');
        }
        return 'specific';
    }
    ...
    {
        if (!is_dir($path)) {
            throw new UcdZend_Loader_Exception('Invalid ZF path provided');
        }
