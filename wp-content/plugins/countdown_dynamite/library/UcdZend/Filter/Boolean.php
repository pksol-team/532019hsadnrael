 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Filter_Boolean implements UcdZend_Filter_Interface
{
    const BOOLEAN      = 1;
    const INTEGER      = 2;
    const FLOAT        = 4;
     ...
     * Constructor
     *
     * @param string|array|UcdZend_Config $options OPTIONAL
     */
    ...
    public function setType($type = null)
    {
        ...
        if (!is_int($type) || ($type < 0) || ($type > self::ALL)) {
            ...
            throw new UcdZend_Filter_Exception('Unknown type');
        }
     ...
     *
     * @param  string|array|UcdZend_Locale $locale
     * @throws UcdZend_Filter_Exception
     * @return UcdZend_Filter_Boolean
     */
    public function setLocale($locale = null)
    {
        if (is_string($locale)) {
            $locale = array($locale);
        } elseif ($locale instanceof UcdZend_Locale) {
            $locale = array($locale->toString());
        } elseif (!is_array($locale)) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Locale has to be string, array or an instance of UcdZend_Locale');
        }

        require_once 'UcdZend/Locale.php';
        foreach ($locale as $single) {
            if (!UcdZend_Locale::isLocale($single)) {
                require_once 'UcdZend/Filter/Exception.php';
                throw new UcdZend_Filter_Exception("Unknown locale '$single'");
            }
        }
    ...
    protected function _getLocalizedQuestion($value, $yes, $locale)
    {
        ...
        $str = UcdZend_Locale::getTranslation($question, 'question', $locale);
        $str = explode(':', $str);
        if (!empty($str)) {
            foreach($str as $no) {
