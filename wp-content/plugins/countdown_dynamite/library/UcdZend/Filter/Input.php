...
/**
 * @see UcdZend_Validate
 */
require_once 'UcdZend/Validate.php';
...
class UcdZend_Filter_Input
{
    ...
    /**
     * @var UcdZend_Filter_Interface The filter object that is run on values
     ...
     * @return UcdZend_Filter_Input
     * @throws UcdZend_Filter_Exception on invalid type
     */
    public function setPluginLoader(UcdZend_Loader_PluginLoader_Interface $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new UcdZend_Filter_Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    ...
    public function getPluginLoader($type)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                case self::FILTER:
                    $prefixSegment = 'UcdZend_Filter_';
                    $pathSegment   = 'UcdZend/Filter/';
                    break;
                case self::VALIDATE:
                    $prefixSegment = 'UcdZend_Validate_';
                    $pathSegment   = 'UcdZend/Validate/';
                    break;
                default:
                    require_once 'UcdZend/Filter/Exception.php';
                    throw new UcdZend_Filter_Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }
    ...
    /**
     * @return UcdZend_Filter_Input
     * @throws UcdZend_Filter_Exception
     */
    public function process()
    {
        ...
        if ($this->hasInvalid()) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception("Input has invalid fields");
        }
        if ($this->hasMissing()) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception("Input has missing fields");
        }
    ...
    public function setDefaultEscapeFilter($escapeFilter)
    {
        ...
        if (!$escapeFilter instanceof UcdZend_Filter_Interface) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Escape filter specified does not implement UcdZend_Filter_Interface');
        }
     ...
     * @param array $options
     * @return UcdZend_Filter_Input
     * @throws UcdZend_Filter_Exception if an unknown option is given
     */
    public function setOptions(array $options)
    {
        foreach ($options as $option => $value) {
            switch ($option) {
                ...
                default:
                    require_once 'UcdZend/Filter/Exception.php';
                    throw new UcdZend_Filter_Exception("Unknown option '$option'");
                    break;
            }
        }
    ...
    public function setTranslator($translator = null)
    {
        if ((null === $translator) || ($translator instanceof UcdZend_Translate_Adapter)) {
            ...
            throw new UcdZend_Validate_Exception('Invalid translator specified');
        }
    ...
    protected function _filter()
    {
        foreach ($this->_filterRules as $ruleName => &$filterRule) {
             ...
             * Don't typecast to (array) because it might be a UcdZend_Filter object
             */
            ...
            if (!isset($filterRule[self::FILTER_CHAIN])) {
                $filterRule[self::FILTER_CHAIN] = new UcdZend_Filter();
    ...
    protected function _validate()
    {
        ...
        foreach ($this->_validatorRules as $ruleName => &$validatorRule) {
             ...
             * Don't typecast to (array) because it might be a UcdZend_Validate object
             */
            ...
            if (!isset($validatorRule[self::VALIDATOR_CHAIN])) {
                $validatorRule[self::VALIDATOR_CHAIN] = new UcdZend_Validate();
                ...
                foreach ($validatorList as $key => $validator) {
                    ...
                    if (isset($validatorRule[self::MESSAGES][$key])) {
                        ...
                        if ($validator instanceof UcdZend_Validate_NotEmpty) {
    ...
    protected function _validateRule(array $validatorRule)
    {
        ...
        if (count((array) $validatorRule[self::FIELDS]) > 1) {
            ...
            if ($validatorRule[self::ALLOW_EMPTY]) {
                ...
                $validatorChain = new UcdZend_Validate();
     ...
     * @param string $type
     * @param mixed $classBaseName
     * @return UcdZend_Filter_Interface|UcdZend_Validate_Interface
     * @throws UcdZend_Filter_Exception
     */
    protected function _getFilterOrValidator($type, $classBaseName)
    {
        ...
        $interfaceName = 'UcdZend_' . ucfirst($type) . '_Interface';
        ...
        if (!$class->implementsInterface($interfaceName)) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception("Class '$className' based on basename '$classBaseName' must implement the '$interfaceName' interface");
        }
