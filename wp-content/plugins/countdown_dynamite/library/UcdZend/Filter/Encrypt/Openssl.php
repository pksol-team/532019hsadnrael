...
/**
 * @see UcdZend_Filter_Encrypt_Interface
 */
require_once 'UcdZend/Filter/Encrypt/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Filter_Encrypt_Openssl implements UcdZend_Filter_Encrypt_Interface
{
    /**
    ...
    public function __construct($options = array())
    {
        if (!extension_loaded('openssl')) {
            ...
            throw new UcdZend_Filter_Exception('This filter needs the openssl extension');
        }
     ...
     * @return UcdZend_Filter_Encrypt_Openssl
     */
    protected function _setKeys($keys)
    {
        if (!is_array($keys)) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Invalid options argument provided to filter');
        }
        ...
        foreach ($keys as $type => $key) {
            ...
            switch ($type) {
                case 'public':
                    ...
                    if ($test === false) {
                        require_once 'UcdZend/Filter/Exception.php';
                        throw new UcdZend_Filter_Exception("Public key '{$cert}' not valid");
                    }
                ...
                case 'private':
                    ...
                    if ($test === false) {
                        require_once 'UcdZend/Filter/Exception.php';
                        throw new UcdZend_Filter_Exception("Private key '{$cert}' not valid");
                    }
    ...
    public function encrypt($value)
    {
        ...
        if (count($this->_keys['public']) == 0) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Openssl can not encrypt without public keys');
        }
        ...
        if ($crypt === false) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Openssl was not able to encrypt you content with the given options');
        }
    ...
    public function decrypt($value)
    {
        ...
        if (count($this->_keys['private']) !== 1) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Openssl can only decrypt with one private key');
        }

        if (empty($envelope)) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Openssl can only decrypt with one envelope key');
        }
        ...
        if ($crypt === false) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('Openssl was not able to decrypt you content with the given options');
        }
