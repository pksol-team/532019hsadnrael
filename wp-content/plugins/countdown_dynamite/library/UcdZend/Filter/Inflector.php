...
/**
 * @see UcdZend_Loader_PluginLoader
 */
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Filter_Inflector implements UcdZend_Filter_Interface
{
    /**
     * @var UcdZend_Loader_PluginLoader_Interface
     */
    ...
    {
        if (!$this->_pluginLoader instanceof UcdZend_Loader_PluginLoader_Interface) {
            $this->_pluginLoader = new UcdZend_Loader_PluginLoader(array('UcdZend_Filter_' => 'UcdZend/Filter/'), __CLASS__);
        }
     ...
     * Set PluginLoader
     *
     * @param UcdZend_Loader_PluginLoader_Interface $pluginLoader
     * @return UcdZend_Filter_Inflector
     */
    public function setPluginLoader(UcdZend_Loader_PluginLoader_Interface $pluginLoader)
    {
    ...
    /**
     * Use UcdZend_Config object to set object state
     *
     ...
     * @param string $spec
     * @param int $index
     * @return UcdZend_Filter_Interface|false
     */
     ...
     *
     * @param string $spec
     * @param array|string|UcdZend_Filter_Interface $ruleSet
     * @return UcdZend_Filter_Inflector
     */
    ...
    public function filter($source)
    {
        ...
        if ($this->_throwTargetExceptionsOn && (preg_match('#(?='.$pregQuotedTargetReplacementIdentifier.'[A-Za-z]{1})#', $inflectedTarget) == true)) {
            ...
            throw new UcdZend_Filter_Exception('A replacement identifier ' . $this->_targetReplacementIdentifier . ' was found inside the inflected target, perhaps a rule was not satisfied with a target source?  Unsatisfied inflected target: ' . $inflectedTarget);
        }
    ...
    {
        if ($rule instanceof UcdZend_Filter_Interface) {
            return $rule;
        }
        ...
        if (!$ruleObject instanceof UcdZend_Filter_Interface) {
            require_once 'UcdZend/Filter/Exception.php';
            throw new UcdZend_Filter_Exception('No class named ' . $rule . ' implementing UcdZend_Filter_Interface could be found');
        }
