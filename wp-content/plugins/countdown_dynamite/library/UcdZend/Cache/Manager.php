...
class UcdZend_Cache_Manager
{
     ...
     *
     * @param  string $name
     * @param  UcdZend_Cache_Core $cache
     * @return UcdZend_Cache_Manager
     */
    public function setCache($name, UcdZend_Cache_Core $cache)
    {
    ...
    public function getCache($name)
    {
        ...
        if (isset($this->_optionTemplates[$name])) {
            if ($name == self::PAGECACHE
                ...
                || !$this->_optionTemplates[$name]['backend']['options']['tag_cache'] instanceof UcdZend_Cache_Core)
            ) {
                $this->_optionTemplates[$name]['backend']['options']['tag_cache']
                    = $this->getCache(self::PAGETAGCACHE);
            }

            $this->_caches[$name] = UcdZend_Cache::factory(
    ...
    public function setCacheTemplate($name, $options)
    {
        if ($options instanceof UcdZend_Config) {
            ...
            require_once 'UcdZend/Cache/Exception.php';
            throw new UcdZend_Cache_Exception('Options passed must be in'
                . ' an associative array or instance of UcdZend_Config');
        }
     ...
     * @param  array $options
     * @return UcdZend_Cache_Manager
     * @throws UcdZend_Cache_Exception for invalid options format or if option templates do not have $name
     */
    public function setTemplateOptions($name, $options)
    {
        ...
        if (!isset($this->_optionTemplates[$name])) {
            throw new UcdZend_Cache_Exception('A cache configuration template'
