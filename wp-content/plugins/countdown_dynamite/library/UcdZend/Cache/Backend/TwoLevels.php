 ...
 * @subpackage UcdZend_Cache_Backend
...
/**
 * @see UcdZend_Cache_Backend_ExtendedInterface
 */
require_once 'UcdZend/Cache/Backend/ExtendedInterface.php';

/**
 * @see UcdZend_Cache_Backend
 */
require_once 'UcdZend/Cache/Backend.php';
...
class UcdZend_Cache_Backend_TwoLevels extends UcdZend_Cache_Backend implements UcdZend_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
     * =====> (string) slow_backend :
     * - Slow backend name
     * - Must implement the UcdZend_Cache_Backend_ExtendedInterface
     ...
     * - See UcdZend_Cache::factory() method
     *
     ...
     * Slow Backend
     *
     * @var UcdZend_Cache_Backend_ExtendedInterface
     */
    ...
    public function __construct(array $options = array())
    {
        ...
        if ($this->_options['slow_backend'] === null) {
            UcdZend_Cache::throwException('slow_backend option has to set');
        } elseif ($this->_options['slow_backend'] instanceof UcdZend_Cache_Backend_ExtendedInterface) {
            $this->_slowBackend = $this->_options['slow_backend'];
        } else {
            $this->_slowBackend = UcdZend_Cache::_makeBackend(
            ...
            if (!in_array('UcdZend_Cache_Backend_ExtendedInterface', class_implements($this->_slowBackend))) {
                UcdZend_Cache::throwException('slow_backend must implement the UcdZend_Cache_Backend_ExtendedInterface interface');
            }
        }

        if ($this->_options['fast_backend'] === null) {
            UcdZend_Cache::throwException('fast_backend option has to set');
        } elseif ($this->_options['fast_backend'] instanceof UcdZend_Cache_Backend_ExtendedInterface) {
            $this->_fastBackend = $this->_options['fast_backend'];
        } else {
            $this->_fastBackend = UcdZend_Cache::_makeBackend(
            ...
            if (!in_array('UcdZend_Cache_Backend_ExtendedInterface', class_implements($this->_fastBackend))) {
                UcdZend_Cache::throwException('fast_backend must implement the UcdZend_Cache_Backend_ExtendedInterface interface');
            }
        }
     ...
     *
     * Available modes are :
     * UcdZend_Cache::CLEANING_MODE_ALL (default)    => remove all cache entries ($tags is not used)
     * UcdZend_Cache::CLEANING_MODE_OLD              => remove too old cache entries ($tags is not used)
     * UcdZend_Cache::CLEANING_MODE_MATCHING_TAG     => remove cache entries matching all given tags
     *                                               ($tags can be an array of strings or a single string)
     * UcdZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries not {matching one of the given tags}
     *                                               ($tags can be an array of strings or a single string)
     * UcdZend_Cache::CLEANING_MODE_MATCHING_ANY_TAG => remove cache entries matching any given tags
     *                                               ($tags can be an array of strings or a single string)
     *
     ...
     * @return boolean true if no problem
     */
    public function clean($mode = UcdZend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch($mode) {
            case UcdZend_Cache::CLEANING_MODE_ALL:
                $boolFast = $this->_fastBackend->clean(UcdZend_Cache::CLEANING_MODE_ALL);
                $boolSlow = $this->_slowBackend->clean(UcdZend_Cache::CLEANING_MODE_ALL);
                return $boolFast && $boolSlow;
                break;
            case UcdZend_Cache::CLEANING_MODE_OLD:
                return $this->_slowBackend->clean(UcdZend_Cache::CLEANING_MODE_OLD);
            case UcdZend_Cache::CLEANING_MODE_MATCHING_TAG:
                ...
                return $res;
                break;
            case UcdZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG:
                ...
                return $res;
                break;
            case UcdZend_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                ...
                break;
            default:
                UcdZend_Cache::throwException('Invalid mode for clean() method');
                break;
        }
    }
