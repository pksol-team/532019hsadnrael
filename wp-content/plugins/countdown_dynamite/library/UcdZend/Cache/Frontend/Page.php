 ...
 * @subpackage UcdZend_Cache_Frontend
...
/**
 * @see UcdZend_Cache_Core
 */
require_once 'UcdZend/Cache/Core.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class UcdZend_Cache_Frontend_Page extends UcdZend_Cache_Core
{
    /**
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_specificOptions['http_conditional'])) {
            if ($this->_specificOptions['http_conditional']) {
                UcdZend_Cache::throwException('http_conditional is not implemented for the moment !');
            }
        }
    ...
    {
        if (!is_array($options)) {
            UcdZend_Cache::throwException('default_options must be an array !');
        }
        foreach ($options as $key=>$value) {
            if (!is_string($key)) {
                UcdZend_Cache::throwException("invalid option [$key] !");
            }
            $key = strtolower($key);
    ...
    {
        if (!is_array($regexps)) {
            UcdZend_Cache::throwException('regexps option must be an array !');
        }
        foreach ($regexps as $regexp=>$conf) {
            if (!is_array($conf)) {
                UcdZend_Cache::throwException('regexps option must be an array of arrays !');
            }
            $validKeys = array_keys($this->_specificOptions['default_options']);
            foreach ($conf as $key=>$value) {
                if (!is_string($key)) {
                    UcdZend_Cache::throwException("unknown option [$key] !");
                }
                $key = strtolower($key);
