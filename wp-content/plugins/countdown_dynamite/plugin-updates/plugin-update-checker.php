 ...
 *
 * Last patch by A.Oliinyk 2015-09-09
 */
...
class PluginUpdateChecker_1_3 {
    // Patch by A.Oliinyk: Prevent multiple messages
    static protected $_displayed;
    ...
    protected function installHooks(){
        ...
        // Patch by A.Oliinyk: support network admin
        add_action('network_admin_notices', array($this, 'displayManualCheckResult'));
    ...
    public function addCheckForUpdatesLink($pluginMeta, $pluginFile, $pluginData = null, $status = null) {
        if ( $pluginFile == $this->pluginFile && current_user_can('update_plugins') ) {
            ...
            // Patch by A.Oliinyk: support WP network admin
            $url = function_exists('is_network_admin') && is_network_admin()
                ? network_admin_url('plugins.php')
                : admin_url('plugins.php');

            $linkUrl = wp_nonce_url(
                add_query_arg('puc_check_for_updates', $this->slug, $url),
    ...
    public function handleManualCheck() {
        ...
        if ( $shouldCheck ) {
            ...
            $url = function_exists('is_network_admin') && is_network_admin()
                ? network_admin_url('plugins.php')
                : admin_url('plugins.php');
            // Patch by A. Oliinyk: remember plugin name
            $url = add_query_arg('plugin', $this->pluginFile, $url);

            wp_redirect(add_query_arg('puc_update_check_result', $status, $url));
        }
    }
    ...
    public function displayManualCheckResult() {
        // Patch by A.Oliinyk: support multisite
        global $current_site;
        ...
        if ( isset($_GET['puc_update_check_result']) ) {
            ...
            if ( $status == 'no_update' ) {
                ...
                if (is_multisite() && !is_main_site()) {
                    if (is_super_admin()) {
                        $active = TRUE;
                        if ((!class_exists('Emb_WpPlugin')
                            || Emb_WpPlugin::VERSION < '03.04.00')
                                && isset($_GET['plugin'])
                        ) {
                            // No Empire Builder or old one
                            switch_to_blog($current_site->blog_id);
                            $active = is_plugin_active($_GET['plugin']);
                            if (!$active) {
                                $pluginAdminUrl = admin_url('plugins.php');
                            }
                            restore_current_blog();
                        }

                        if ($active) {
                            $message .= ' Please go to <a href="'
                                . network_admin_url('plugins.php') . '">network admin</a> to update the plugin.';
                        } else {
                            $message .= ' Please go to <a href="'
                                . $pluginAdminUrl . '">main site admin</a> and activate the plugin then go to <a href="'
                                . network_admin_url('plugins.php') . '">network admin</a> to update it.';
                        }

                    } else {
                        $message .= ' Please notify your multisite network administrator.';
                    }
                }
            } else {
