	...
	final protected function sanitize_setting( $type, $var, $name, $prevent_save_on_error = true, $trim_value = true ) {
		...
		if ( 'string' === $type ) {
			...
			if ( is_array( $this->settings[$var] ) ) {
				...
				$error = wp_sprintf( _n( 'The valid value for %1$s is: %2$l.', 'The valid values for %1$s are: %2$l.', count( $roles ), 'it-l10n-ithemes-security-pro' ), $name, $roles );
				$type = 'array';
			}
			...
			if ( ! is_array( $this->settings[ $var ] ) ) {
				$error = sprintf( __( 'The %1$s value must be an array.', 'it-l10n-ithemes-security-pro' ), $name );
			} else {
				$invalid_entries = array();
