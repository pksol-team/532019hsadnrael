...
final class ITSEC_Notification_Center {
	...
	public function display_notification_center_link_for_module( $module_slug ) {
		...
		if ( $display ) {
			...
			echo '<a href="' . $href .'" class="itsec-notification-center-link" data-module-link="notification-center">' . esc_html__( 'Notification Center', 'it-l10n-ithemes-security-pro' ) . '</a>';
		}
	}
	...
	public function check_notification_schedule_accurate( $notifications = array() ) {
		...
		if ( ! ITSEC_Lib::get_lock( 'notification-center', 120 ) ) {
			return new WP_Error( 'itsec-notification-center-cannot-get-lock', esc_html__( 'Cannot get lock.', 'it-l10n-ithemes-security-pro' ) );
		}
	...
	public static function get_schedule_labels() {
		return array(
			self::S_DAILY   => esc_html__( 'Daily', 'it-l10n-ithemes-security-pro' ),
			self::S_WEEKLY  => esc_html__( 'Weekly', 'it-l10n-ithemes-security-pro' ),
			self::S_MONTHLY => esc_html__( 'Monthly', 'it-l10n-ithemes-security-pro' ),
		);
	}
