...
class ITSEC_Fingerprinting {
	...
	public function run_restrict_capabilities() {
		...
		add_filter( 'user_has_cap', array( $this, 'restrict_capabilities' ), 10, 4 );
	...
	public function attach_fingerprint_to_session( $info, $user_id ) {
		...
		if ( $hash = $fingerprint->calculate_hash() ) {
			$info['itsec_fingerprint_hash'] = $fingerprint->calculate_hash();
		}
	 ...
	 * @return array
	 */
	public function restrict_capabilities( $all_caps, $required_caps, $args, $user ) {
		...
		remove_filter( 'user_has_cap', array( $this, 'restrict_capabilities' ) );
	...
	public function handle_ajax_action() {
		...
		if ( ! ITSEC_Lib_Fingerprinting::is_current_fingerprint_safe() ) {
			...
			wp_send_json_error( array(
				'message' => esc_html__( "You're current device is unconfirmed, so you do not have permission to approve new devices. Check your email for a link to approve this current device.", 'it-l10n-ithemes-security-pro' )
			) );
		}
