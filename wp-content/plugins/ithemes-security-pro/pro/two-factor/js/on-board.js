(function ( $, Backbone, _, wp, config ) {
	 ...
	 */
	function ajax( data ) {
		return wp.ajax.post( 'itsec-login-interstitial-ajax', _.extend( {}, {
			itsec_interstitial_action: '2fa-on-board',
			itsec_interstitial_user  : $( 'input[name="itsec_interstitial_user"]' ).val(),
			itsec_interstitial_token : $( 'input[name="itsec_interstitial_token"]' ).val(),
			itsec_after_interstitial : $( 'input[name="itsec_after_interstitial"]' ).val(),
		}, data ) );
	}
...
})( jQuery, window.Backbone, window._, window.wp, window['ITSEC2FAOnBoard'] );