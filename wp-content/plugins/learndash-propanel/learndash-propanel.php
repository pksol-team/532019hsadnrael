...
class LearnDash_Propanel{
	...
	function learndash_propanel_dashboard_overview() {
		$email_message = LearnDash_Propanel::send_dashboard_emails($course_progress_data);
	...
	static function send_dashboard_emails($course_progress_data = null) { //return;
		...
		if(empty($course_progress_data[$course_id]["users"]))
			return;
		$users = $course_progress_data[$course_id]["users"];
    	$email_to = array();
	...
	static function learndash_propanel_csv_export() {
		...
		if(empty($_GET['learndash_propanel_csv_export']))
			return;
	    ...
	    $query = get_users(); //use the get_users call
	    $courses = get_pages("post_type=sfwd-courses");

	    $user_display = array();
	    foreach ($query as $user) {
	        # code...
	        $learndash_propanel_user_meta = get_user_meta($user->data->ID);
	        $array_keys = array_keys($learndash_propanel_user_meta);
	    ...
	    $final_data = array();
	    foreach ($user_display as $data => $value) {
	        $progress = learndash_course_progress(array("user_id" => $value["user_id"], 'course_id' => $value["course_id"], "array" => true));
	        $final_data[] = array('user_id'=> $value["user_id"], 'user_name' => $value["username"], 'user_email' => $value["email"], 'course_id'=> $value["course_id"], 'progress' => $progress);
	    }
	    $course_details_data = array();
	    foreach ($final_data as $item) {
	        $course_id    = $item['course_id'];
	        $user_id      = $item['user_id'];
	        $user_email   = $item['user_email'];
	        $username     = $item['user_name'];
	        $complete     = $item['progress']['completed'];
	        $final        = $item['progress']['total'];
	        $post         = get_post($course_id, ARRAY_A);
	        $course_title = $post['post_title'];

	        $course_details_data[$course_id]['course_id']    = $course_id;
	        $course_details_data[$course_id]['course_title'] = $course_title;

	        $course_details_data[$course_id]['user_id'][]    = $user_id;
	        $course_details_data[$course_id]['user_email'][] = $user_email;
	        $course_details_data[$course_id]['user_name'][]  = $username;
	        $course_details_data[$course_id]['completed'][]  = $complete;
	        $course_details_data[$course_id]['total'][]      = $final; 
	    }
	    $course_details_data = array_values($course_details_data);
	    //var_dump($course_details_data);
	    // A name with a time stamp, to avoid duplicate filenames
	    $result = array();
	    
	    foreach ($course_details_data as $data) {   
	            if( $_POST['learndash_propanel_course_id'] == $data['course_id'] ){
	                $filename = LearnDash_Propanel::sanitize_filename($data['course_title'])."-user-info.csv";

	                for ($i=0; $i < count($data['user_id']); $i++) { 
	                    $result[] = array(
	                        "Username" => $data['user_name'][$i],
	                        "User Email" =>$data['user_email'][$i],
	    ...
	    // Extracts the keys of the first record and writes thempi
